  ...
  return (
    <div className="glass-panel">
      <div className="glass-panel-head">
        <h2>لجان التحكيم</h2>
        <button className="btn-action add" onClick={() => setShowForm(true)}>
          <PlusIcon /> إضافة لجنة
        </button>
      </div>
          ...
          {committees.map(c => (
            <div key={c.id} className="committee-card" style={{
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {(c.members || []).map(m => (
                  <span key={m.id} className={`teacher-badge${m.is_head ? '' : ''}`} style={{
      ...
      {showForm && (
        <Modal title="إنشاء لجنة تحكيم جديدة" onClose={() => setShowForm(false)}>
          <form onSubmit={handleCreate}>
            ...
            <div className="field">
              <label>اختيار المحفّظين للجنة (عضوين فأكثر)</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {teachers.map(t => {
